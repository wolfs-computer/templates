...
x11_data get_x11_data() {
    ...
    s.gfx_display = gfx_display;
    s.gfx_window = gfx_window;
    s.gfx_gc = gfx_gc;
    s.gfx_colormap = gfx_colormap;
    s.gfx_fast_color_mode = gfx_fast_color_mode;
...
void check_events(XEvent *event) {
    ...
    if ((*event).type == Expose) {
        // Draw only the exposed region
        XExposeEvent *expose = (XExposeEvent*) event;
        XFillRectangle(gfx_display, gfx_window, gfx_gc, expose->x, expose->y, expose->width, expose->height);
    }

    // Handle KeyPress event to exit the loop
    if ((*event).type == KeyPress) {
        exit(0);
    }
